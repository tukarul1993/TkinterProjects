...
def update_data(row):
    ...
    conn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};'
                          ...
                          'PWD=admin@123')
    cursor = conn.cursor()
    print("update pyDE_Students set Name=?, Age=? where Name =?",
                   (entry1.get(), entry2.get(),entry1.get()))
...
def load_data():
    ...
    for i, row in enumerate(rows):
        ...
        edit_button = tk.Button(grid, text="Edit", command=lambda row=row: update_data(row))
